...
@section('content')
    ...
    <div class="col-md-12">
        <div class="box box-default box-solid {{ request('search') ? '' : 'collapsed-box' }}">
            ...
            <div class="box-body" style="display: {{ request('search') ? 'block' : 'none' }};">
                <form role="form" action="{{ route('backend.employerjobseekers') }}">
        ...
        <div class="dropdown pull-right">
            ...
            <ul class="dropdown-menu" aria-labelledby="sortingMenu">
                <li><a href="
                    ...
                    {{ route('backend.employerjobseekers', array_merge(request()->except('sort'), ['sort' => 'created_at'])) }}
                    @elseif ( request()->get('sort') == 'created_at' )
                    {{ route('backend.employerjobseekers', array_merge(request()->except('sort'), ['sort' => 'likes'])) }}
                    @else
                    {{ route('backend.employerjobseekers', array_merge(request()->except('sort'), ['sort' => 'likes'])) }}
                    @endif
                            ">
    ...
    <table class="table table-striped table-bordered table-hover">
        ...
        @foreach($job_seekers as $job_seeker)
            ...
            <tr>
                <td><a href="{{ route('backend.employerjobseekers.show' , [ $job_seeker->id ] ) }}">{{ $job_seeker->user->name }}</a>
                </td>
                <td><a href="{{ route('backend.employerjobseekers', ['size' => $job_seeker->size]) }}">
                        {{ str_studly($job_seeker->size) }}
                    </a></td>
                <td><a href="{{ route('backend.employerjobseekers', ['country_id' => $job_seeker->country->id]) }}">
                        {{ $job_seeker->country->name }}
                    </a></td>
                <td><a href="{{ route('backend.employerjobseekers', ['state_id' => $job_seeker->state->id]) }}">
