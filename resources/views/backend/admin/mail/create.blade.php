...
@section('content')
    ...
    {!! Form::open(['route' => 'admin.mail.send', 'class' => 'form-horizontal', 'role' => 'form', 'method' => 'post']) !!}
        ...
        <div class="col-lg-5">
            <select name="company" id="company" class="form-control">
                ...
                @foreach($companies as $company)
                    <option {{ ( (!empty(old('company'))) && old('company') == $company->id ) ? 'selected="selected"' : '' }} value="{{ $company->id }}">{{ $company->title }}</option>
                @endforeach
            </select>
        </div>
        <div class="col-lg-5">
            <select name="to" id="to" class="form-control">
                ...
                @if($to_users)
                    @foreach($to_users as $to_user)
                        <option {{ ( (!empty(old('to'))) && old('to') == $to_user->id ) ? 'selected="selected"' : '' }} value="{{ $to_user->id }}">
                            {{ $to_user->name }}
                        </option>
                    @endforeach
                @endif
    ...
    <div class="form-group">
        ...
        <div class="col-lg-10">
            <textarea name="message" id="message" cols="30" rows="10" class="form-control textarea">{{ old('message') }}</textarea>
        </div>
    </div>
